...
object BoardComponent {
  ...
  val create: Component[Future, State, Params, Event] =
    Component[Future, State, Params, Event](State.Static(None, None, 0)) { (context, parameters, state) =>
      ...
      def neighbours(p: Point): List[Point] = {
        List(p.left, p.right, p.top, p.bottom, p.top.left, p.top.right, p.bottom.left, p.bottom.right)
      }
      ...
      (parameters, state) match {
            ...
            cellClick = p2 =>
              Some(access => {
                ...
                swapOpt match {
                  ...
                  case None if selectedCellOpt.contains(p2) =>
        ...
        case (_, State.AnimationEnd(an, board, batch)) =>
          ...
          renderStaticBoard(board, None, None, Nil)(
            cellClick = _ => None,
            cellSwipe = (_, _) => None,
            onAnimationEnd = Some { access: Access =>
              {
                if (batch.isEmpty) {
                  for {
                    _ <- access.transition(_ => State.Static(Some(board), None, an))
