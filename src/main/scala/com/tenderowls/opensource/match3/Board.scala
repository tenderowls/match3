...
object Board {
  ...
  class Board(val rules: Rules, val data: BoardData) extends Immutable {
    ...
    /**
      ...
      * @return
      */
    def matchedSequences(startLength: Int = 3): Iterable[List[MatchedCell]] = {
    ...
    def applyOperations(operations: List[BoardOperation]): Board = {
      @tailrec
      def mutateCellAt(point: Point, list: List[BoardOperation]): Cell =
        list match {
          case x :: xs =>
            x match {
      ...
      val cells = mapData((point, i) => mutateCellAt(point, reverseOperations))
